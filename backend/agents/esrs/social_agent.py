...
def social_agent(state: GraphState) -> GraphState:
    ...
    if "all" not in response.model_dump() or not response.model_dump().get("all"):
        raise ValueError("No social materiality assessment breakdowns found in the response.")
    ...
    return {
        **state,
        "is_fan_out": False,
        "messages": [AIMessage(content="Succesfully completed for the social report.")],
        "current_step": "social",
    }
