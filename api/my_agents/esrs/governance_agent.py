...
def governance_agent(state: GraphState) -> GraphState:
    ...
    is_from_tool_node = steps.get("governance") is True

    response = None
    if not is_from_tool_node:
        governance_tools = governance.bind_tools([web_search_tool])
        response = governance_tools.invoke(messages)
    ...
    if is_from_tool_node:
        ...
        extract_messages = messages + [
            HumanMessage(
                content="Please extract the Governance Assessment Breakdowns now. You must return the appropriate tool call for the extraction."
            )
        ]
        result = trustcall.invoke(extract_messages)
        if not result.get("responses"):
            raise ValueError(
                "No governance materiality assessment breakdowns found in the response."
            )
